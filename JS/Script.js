...
let $userImg = document
  .querySelector(".user")
  .addEventListener("click", userInformation);
let $toSchedule = document
  .querySelector(".toSchedule")
  .addEventListener("click", toggleEments);
  ...
  document.querySelector("form").addEventListener("submit", function (event) {
    ...
    if (isNameValid && isEmailValid && isDateValid && isTypeValid) {
      ...
      setTimeout(function () {
        ...
        let existentConsult = consultDados.find(
          (user) => user.email === userDados.email || consultDados == ""
        );
        if (!existentConsult) {
