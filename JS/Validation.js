...
function validarFormulario() {
    ...
    if (input1Value === '' || !input1Value.includes('@') || !input1Value.includes('.com')) {
...
let button = document.getElementById('btn').addEventListener('click',validarFormulario);
...
let myForm = document.querySelector('#myForm').addEventListener('submit', function (event){
    ...
    let carregando = document.querySelector('.carregando').style.display = "block";

    let input2Value = document.getElementById('password').style.border = "1px solid green";
    ...
    setTimeout (function timeoutFunction (){
        ...
        function verificarLoginNoLocalStorage(event) {
            ...
            let usuarioExistente = users.find(user => user.email === email && user.password === password);
